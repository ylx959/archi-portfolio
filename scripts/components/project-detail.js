...
export function openProjectDetail(index, sourceCard) {
    ...
    // The stage is deliberately not built here. It opens in detail mode, so
    // .project-detail-gallery-mode is display:none and a render measures a
    // zero-width viewport — it produced a translate of 0 and threw it away.
    // What it did cost was two DOM nodes per gallery image, three times over,
    // in the same frames the open transition is trying to start. The toggle
    // into gallery mode calls scheduleProjectGalleryModeRender() itself.
...
function closeProjectDetail() {
    ...
    projectDetailCloseTimer = window.setTimeout(function () {
        ...
        // Drop the stage after the fade, not during it: a long gallery holds
        // tens of megabytes of decoded image, and leaving it attached means the
        // next project opens on top of it.
        if (projectDetailStageTrack) {
            projectDetailStageTrack.innerHTML = "";
        }
