...
function setupContactRippleField() {
    ...
    let lastSpawnX = 0;
    let lastSpawnY = 0;
    // what the current grid was built for, so a rebuild that would change
    // nothing costs a comparison instead of three typed arrays
    let builtWidth = 0;
    let builtHeight = 0;
    let builtPixelRatio = 0;
    ...
    function buildContactDots() {
        ...
        if (width === builtWidth && height === builtHeight && pixelRatio === builtPixelRatio) {
            return;
        }

        builtWidth = width;
        builtHeight = height;
        builtPixelRatio = pixelRatio;
    ...
    function drawContactField() {
        ...
        // How far a crest grows a dot, and how far it shoves one sideways. They
        // are the two knobs that decide how loud the wave reads: swell was 1.55
        // here, which took a 0.82px dot past 2.3px and turned the wavefront into
        // a band of blobs rather than a ripple running through a grid.
        const swell = compact ? 0.72 : 0.95;
    ...
    // The section's own box drifts long after this first measurement — fonts
    // land, the sticky stack writes its transform, the layout settles — and none
    // of that fires a window resize. Measuring only on `resize` left the canvas
    // at whatever size the section happened to be at DOMContentLoaded, and a
    // canvas shorter than the section is a field of dots that stops partway down
    // it. Observing the box itself is the only thing that sees those changes;
    // the resize listener stays for the one it cannot, a devicePixelRatio change
    // from dragging the window onto another display.
    //
    // border-box, because offsetHeight above is one: the default content box
    // does not move when the section's own padding does, and that padding is
    // written from --section-overlap, a vh value.
    if ("ResizeObserver" in window) {
        new ResizeObserver(buildContactDots).observe(contactSection, { box: "border-box" });
    }
