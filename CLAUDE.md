...
- **Enter gate** (`components/hero.js`). `html`/`body` start with `is-locked` (page blurred, scroll disabled) until the name form is submitted. Until then, in-page anchor links are inert. The form itself does not appear with the copy: `completeHeroExpand()` waits on the description scramble and only then calls `revealEnterForm()`, because an input offered while the sentence is still noise pulls the eye to the caret and the copy goes unread. That wait is raced against `enterFormTimeout` — if the scramble ever failed to resolve the form would never appear and the site could not be entered at all.
