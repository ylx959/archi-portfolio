...
- **Hero intro choreography** (`components/hero.js`). A promise chain: status lines scramble in and out, the card drops, the headline drops, the ampersand hops in. Each hand-off to another component goes through `dropHeroPart(target[, timeout])`, which dispatches `portfolio:hero-drop` and resolves on the matching `portfolio:hero-dropped` — so a new piece of the intro is a new target, not a new import.
